...
export default {
    ...
    storage: multer.diskStorage({
        ...
        filename(request, file, callback){
            const fileHash = crypto.randomBytes(10).toString('hex'); //gera um hash de 10 bytes em formato hexadecimal
            const fileName = `${fileHash}-${file.originalname}`; //nome do arquivo, juntando o nome original com o hash gerado, para ser único
